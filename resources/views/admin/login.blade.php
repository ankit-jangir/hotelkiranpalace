...
<body>
                    ...
                    <div class="col-lg-5 col-md-7 col-12 px-3 px-md-4">
                        <div class="admin-login-card">
                            <div class="admin-login-logo">
                                <img src="{{ asset('images/logo.png') }}" alt="Hotel Kiran Place Logo"
                                    onerror="this.onerror=null; this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2250%22 height=%2250%22%3E%3Crect width=%2250%22 height=%2250%22 fill=%22%23ff6b35%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 fill=%22%23fff%22 font-size=%2225%22 font-weight=%22bold%22%3EH%3C/text%3E%3C/svg%3E'; this.style.display='inline-block';">
                            </div>
                            ...
                            <form action="{{ route('admin.login.submit') }}" method="POST" id="adminLoginForm"
                                ...
                                <div class="admin-form-group">
                                    ...
                                    <div class="position-relative">
                                        <input type="email" class="admin-form-input" id="username" name="username"
                                            value="{{ old('username') }}" placeholder="Enter your email" required
                                            autocomplete="email">
                                ...
                                <div class="admin-form-group">
                                    ...
                                    <div class="position-relative">
                                        ...
                                        <i class="fas fa-eye admin-input-icon admin-password-toggle"
                                            id="passwordToggle"></i>
                                    </div>
                                ...
                                <div id="rateLimitTimer" class="admin-rate-limit-timer" style="display: none;">
                                    ...
                                    <span id="timerText">Please wait <span id="timerSeconds">60</span> seconds before
                                        trying again.</span>
                                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showToast(message, type = 'error') {
            if (typeof window.toast === 'function') {
                ...
                toastEl.className =
                    `toast align-items-center text-white bg-${type === 'error' ? 'danger' : 'success'} border-0`;
        ...
        function startRateLimitTimer(seconds) {
            ...
            const timerInterval = setInterval(function() {
                ...
                if (remainingSeconds > 0) {
                    if (mins > 0) {
                        timerText.innerHTML =
                            `Please wait <span id="timerSeconds">${mins} minute${mins > 1 ? 's' : ''} ${secs} second${secs !== 1 ? 's' : ''}</span> before trying again.`;
                    } else {
                        timerText.innerHTML =
                            `Please wait <span id="timerSeconds">${secs} second${secs !== 1 ? 's' : ''}</span> before trying again.`;
                    }
                } else {
                    ...
                    timerText.innerHTML =
                        'Please wait <span id="timerSeconds">60</span> seconds before trying again.';
                }
            }, 1000);
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const showToastMessage = (typeof window.toast === 'function') ? window.toast : (typeof toast ===
            'function' ? toast : null);

        if (showToastMessage) {
            @if(session('success'))
            showToastMessage('success', '{{ session('
                success ') }}');
            @endif

            @if(session('error'))
            showToastMessage('error', '{{ session('
                error ') }}');
            @endif

            @if(session('warning'))
            showToastMessage('warning', '{{ session('
                warning ') }}');
            @endif

            @if(session('info'))
            showToastMessage('info', '{{ session('
                info ') }}');
            @endif
        }
    });
