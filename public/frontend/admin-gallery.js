...
function toggleUploadType(type) {
    ...
    const multipleInputs =
        multipleSection.querySelectorAll('input[type="file"]');
...
function previewMultipleImage(input, wrapper) {
    const previewItem = wrapper.querySelector(
        ".admin-gallery-image-preview-item",
    );
...
function closeAllModals() {
    ...
    document
        .querySelectorAll(".admin-gallery-action-dropdown")
        .forEach((dropdown) => {
...
function openGalleryPanel() {
    ...
    setTimeout(() => {
        ...
        document.querySelector(
            'input[name="upload_type"][value="single"]',
        ).checked = true;
        ...
        fetch(`/admin/gallery/${id}`)
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    const content =
                        document.getElementById("viewGalleryContent");
                    ...
                    if (gallery.type === "video") {
                        // Video display
                        const videoUrl = gallery.main_image
                            ? gallery.main_image.startsWith("http")
                                ? gallery.main_image
                                : `/${gallery.main_image}`
                            : "";
                        if (videoUrl) {
                        ...
                        if (gallery.main_image) {
                            const mainImageUrl = gallery.main_image.startsWith(
                                "http",
                            )
                                ? gallery.main_image
                                : gallery.main_image.startsWith("storage/")
                                  ? `/storage/${gallery.main_image.replace("storage/", "")}`
                                  : `/${gallery.main_image}`;
                            allImages.push({
                                url: mainImageUrl,
                                isMain: true,
                        ...
                        if (
                            gallery.sub_images &&
                            Array.isArray(gallery.sub_images)
                        ) {
                        ...
                        if (allImages.length > 0) {
                            ...
                            const fileUrl = gallery.file_path.startsWith("http")
                                ? gallery.file_path
                                : `/${gallery.file_path}`;
                    ...
                    document
                        .getElementById("viewGalleryModalOverlay")
                        .classList.add("show");
                    document
                        .getElementById("viewGalleryModal")
                        .classList.add("show");
                    document.body.style.overflow = "hidden";
                } else {
...
function changeMainImage(url, index) {
    ...
    document
        .querySelectorAll(".admin-gallery-view-thumbnail")
        .forEach((thumb, i) => {
            if (i === index) {
    ...
    if (type === "image") {
        ...
        if (uploadType === "single") {
            const singleImage = document.querySelector(
                '#singleImageSection input[type="file"]',
            ).files[0];
            if (singleImage) {
                formData.append("images[]", singleImage);
            }
        } else {
            const multipleInputs = document.querySelectorAll(
                '#multipleImagesSection input[type="file"]',
            );
