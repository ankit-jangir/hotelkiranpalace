...
// ========================================
// Global Toast Function - available as both window.toast and toast
function toastFunction(type, message, duration = 5000) {
...
// Make toast available both as window.toast and global toast
window.toast = toastFunction;

// Also create a global toast variable for compatibility
// This allows code to call toast() directly (not just window.toast())
// Use a simple assignment that works in browser environments
if (typeof window !== 'undefined') {
    window.toast = toastFunction;
    // In browser, window properties are accessible as globals in most cases
    // But to be explicit, we'll also try to set it as a global
    try {
        // This works in non-strict mode
        if (typeof toast === 'undefined') {
            // Create a script that declares toast globally
            (function() {
                var globalScope = (function() { return this; })();
                if (globalScope && typeof globalScope.toast === 'undefined') {
                    globalScope.toast = toastFunction;
                }
            })();
        }
    } catch(e) {
        // If that fails, window.toast will still work
        // Code should check for window.toast
    }
}
...
if (roomGalleryModal) {
    const galleryThumbnails =
        roomGalleryModal.querySelectorAll("image.png.gallery-thumbnail");
...
// Admin-related JavaScript has been moved to separate files:
// - admin-blog.js for blog functionality
